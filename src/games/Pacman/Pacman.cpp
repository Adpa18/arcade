...
** Last update	Wed Mar 16 13:26:32 2016 Axel Vencatareddy
*/
                                                        ...
                                                        &comp)
{
  ...
  comp.push(new GameComponent(phantPos, Vector2(STEP, STEP), BLACK,
...
std::stack<AComponent*>		Pacman::compute(int key)
{
  ...
  if ((dir[pacman] == WAIT && (key == RIGHT || key == LEFT || key == DOWN || key == UP))
      || dir[pacman] != WAIT)
    comp.push(new GameComponent(pacPos, Vector2(STEP, STEP), BLACK,
                                std::string(), std::string(), NULL));
  switch (key) {
    case RIGHT:
  ...
  if (collision() == true)
    {
      for (size_t i = 0; i < phantom.size(); i++) {
        comp.push(new GameComponent(phantom[i]->getPos(), Vector2(STEP, STEP),
                                    BLACK, std::string(), std::string(), NULL));
      }
      phantom.clear();
      restart();
    }
...
void				Pacman::restart()
{
  this->pacman = new GameComponent(Vector2(0, 0), Vector2(STEP, STEP),
                                           YELLOW, "", "", NULL);
  this->phantom.push_back(new GameComponent(Vector2(60, 60),
                                            Vector2(STEP, STEP), GREEN, "", "",
                                            NULL));
  this->phantom.push_back(new GameComponent(Vector2(80, 80),
                                             Vector2(STEP, STEP), RED,
                                             "", "", NULL));
  this->phantom.push_back(new GameComponent(Vector2(100, 100),
                                             Vector2(STEP, STEP), CYAN,
                                             "", "", NULL));
  this->phantom.push_back(new GameComponent(Vector2(120, 120),
                                             Vector2(STEP, STEP), WHITE,
                                             "", "", NULL));
