...
** Last update	Wed Mar 16 21:42:52 2016 Nicolas Constanty
*/

#include "Pacman.hpp"

Pacman::Pacman() : AGame("Pacman", Vector2(WIDTH, HEIGHT))
{
  this->restart();
}
...
bool				Pacman::tryDown(Vector2 pos)
{
...
bool				Pacman::tryRight(Vector2 pos)
{
  if (pos.x + STEP >= WIDTH)
    return (false);
  else
...
bool				Pacman::tryLeft(Vector2 pos)
{
  if (pos.x - STEP <= 0)
    return (false);
  else
...
bool				Pacman::collision()
{
  for (size_t i = 0; i < this->phantom.size(); i++) {
    Vector2	tmp = this->phantom[i]->getPos();
...
void				Pacman::phantom_compute(GameComponent *phant,
                                                        ...
                                                        &comp)
{
  Vector2			phantPos = phant->getPos();

  comp.push(new GameComponent(phantPos, Vector2(STEP, STEP), BLACK,' ', "", NULL));
  if (dir[phant] == WAIT)
...
std::stack<AComponent*>		Pacman::compute(int key)
{
  ...
  Vector2			pacPos = this->pacman->getPos();

  if ((dir[pacman] == WAIT && (key == RIGHT || key == LEFT || key == DOWN || key == UP))
      || dir[pacman] != WAIT)
    comp.push(new GameComponent(pacPos, Vector2(STEP, STEP), BLACK, ' ', "", NULL));
  switch (key) {
    case RIGHT:
  ...
  if (collision() == true)
    {
      for (size_t i = 0; i < phantom.size(); i++) {
        comp.push(new GameComponent(phantom[i]->getPos(), Vector2(STEP, STEP), BLACK, ' ', "", NULL));
      }
      phantom.clear();
      restart();
    }
...
void				Pacman::restart()
{
  this->pacman = new GameComponent(Vector2(0, 0), Vector2(STEP, STEP), YELLOW, ' ', "", NULL);
  this->phantom.push_back(new GameComponent(Vector2(60, 60), Vector2(STEP, STEP), GREEN, ' ', "", NULL));
  this->phantom.push_back(new GameComponent(Vector2(80, 80), Vector2(STEP, STEP), RED, ' ', "", NULL));
  this->phantom.push_back(new GameComponent(Vector2(100, 100), Vector2(STEP, STEP), CYAN, ' ', "", NULL));
  this->phantom.push_back(new GameComponent(Vector2(120, 120), Vector2(STEP, STEP), WHITE, ' ', "", NULL));
